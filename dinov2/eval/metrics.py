...
def build_topk_accuracy_metric(average_type: AccuracyAveraging, num_classes: int, ks: tuple = (1, 5)):
    ...
    if num_classes == 1:
        for k in ks:
            metrics[f"top-{k}"] = torchmetrics.classification.BinaryAccuracy(pos_label=1)
    else:
        for k in ks:
            metrics[f"top-{k}"] = MulticlassAccuracy(top_k=k, num_classes=num_classes, average=average_type.value)
